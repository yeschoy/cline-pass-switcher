"""Integrated-console A/B with the archived UI acceptance workload.
Run from repo root against the archived pricing fixture's loopback URL:
python3 .trellis/tasks/09-25-pricing-console-keys-300rpm/research/integrated-ui-browser-check.py http://127.0.0.1:<port>/ dense
Set CPS_UI_SOURCE_REF=cc17ac7 to serve the pre-multi-key committed HTML against the same current server. This is an isolated same-fixture UI comparison, not a production benchmark.
"""
import copy
import json
import os
...
with sync_playwright() as pw:
    ...
    source_ref = os.environ.get('CPS_UI_SOURCE_REF')
    if source_ref:
        assert re.fullmatch(r'[0-9a-f]{7,40}', source_ref), 'explicit commit only'
        repo = next((p for p in pathlib.Path(__file__).resolve().parents if (p / 'server.js').is_file() and (p / '.trellis').is_dir()), None)
        assert repo is not None, 'repository root not found for isolated browser check'
        html = subprocess.check_output(['git', 'show', f'{source_ref}:public/index.html'], cwd=repo)
        page.route(sys.argv[1], lambda route: route.fulfill(body=html, content_type='text/html; charset=utf-8'))
    ...
    if not dense:
        ...
        width['filterRows'] = 10
    print(json.dumps({'kind': 'ui-split-browser-dense' if dense else 'ui-split-browser-smoke', 'htmlRef':source_ref or 'integrated',
