...
后续关联任务：`../09-21-low-quota-pool-refresh-cooling/` 将在本任务的 min/max/persisted target owner 上增加 high/low quota role composition。为避免重复 target、成员表或扩容器，本任务只建立总池 target 与并发触发 grow-one；其基础 priority/ID membership 是后续 role-aware membership 的输入，而不是禁止后续按 quota role 扩展。
...
5. 本任务的基础成员按 hard eligibility、非 reserve、priority、stable ID 选择，先正式入 active 再 lease；不再是一次性 standby overflow。后续 high/low quota composition 可在同一 membership owner 内先分角色，再以 priority/ID 作为稳定 tie-break，不得新建第二成员表。
6. 并发下降不自动缩容。operator 显式修改 min/max/关闭可 clamp；硬状态退出由合格成员替换但 target 不降。
7. 账号成功率永不改变 membership。启用 healthSort 时只排序当前 active；sticky 在同 rate/role 内稳定选择。扩容 target 与硬状态替换仍由本任务 owner 管理，后续 quota role任务只扩展成员派生与角色内选择。
...
- [ ] membership 不受成功率波动影响；healthSort 只排序 active 候选；后续 quota role扩展仍复用同一target/membership owner且不创建第二成员表。
