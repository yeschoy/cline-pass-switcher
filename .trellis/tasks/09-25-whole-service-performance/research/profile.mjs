// Synthetic local-only source-section profiler. Never reads the default DATA_DIR.
// Run: env -u CLINE_PASS_KEY -u PROXY_KEY -u PUBLIC_BASE_URL -u PORT node .trellis/tasks/09-25-whole-service-performance/research/profile.mjs [--ref=<git-commit>] [--extended]
...
const mock = http.createServer((req, res) => {
  const chunks = []; req.on('data', chunk => chunks.push(chunk)); req.on('end', () => {
    ...
    if (body.model === 'synthetic-fail' || (body.model === 'synthetic-retry' && mockHits.chat % 3 !== 0)) { res.writeHead(503, { 'Content-Type': 'application/json' }); return res.end('{"error":{"message":"synthetic unavailable"}}'); }
    if (body.stream) {
...
try {
  ...
  const repeat = async (n, fn) => { const rows=[]; for (let i=0;i<n;i++) rows.push(await fn(i)); return quantiles(rows); };
  const sample = async label => {
    ...
    const coldModels = await read('/api/models');
    ...
    const quota = await timed('/api/statistics/quota-refresh',{method:'POST',headers:fixtureHeaders(port,'/api/statistics/quota-refresh','POST',{'Content-Type':'application/json'}),body:'{"force":true}'});
    const logs = await read('/api/logs/requests?limit=20');
  ...
  if (process.argv.includes('--extended')) {
    ...
    mockHits.chat = Math.ceil(mockHits.chat/3)*3;
    ...
    const sse = await chat('synthetic-slow-sse',100,{'x-sse':'yes'});
    const quota = await timed('/api/statistics/quota-refresh',{method:'POST',headers:fixtureHeaders(port,'/api/statistics/quota-refresh','POST',{'Content-Type':'application/json'}),body:'{"force":true}'});
    const settings = async body => timed('/api/logs/settings',{method:'POST',headers:fixtureHeaders(port,'/api/logs/settings','POST',{'Content-Type':'application/json'}),body:JSON.stringify(body)});
    await settings({errorDetailLogging:true});
    const errorOnlySuccess = await chat(models[0],100);
    const errorOnlyFailure = await chat('synthetic-fail',100);
    await settings({errorDetailLogging:false,detailedLogging:true});
    const full = await chat(models[0],512*1024);
    const detailHealth = await read('/api/logs/settings');
    const errorLogs = await read('/api/logs/errors?limit=20');
    const detailLogs = await read('/api/logs/details?limit=20');
    await settings({detailedLogging:false});
    const large = await chat(models[0],49*1024*1024);
