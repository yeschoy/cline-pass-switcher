...
- Security UI lists key labels and account assignments, supports safe create/copy-once/rotate/revoke confirmation, and preserves hidden account fields across filters/drafts/full saves; show that a nonempty `PROXY_KEY` overrides the legacy key **on startup/restart** even if an admin save changed the running value. Existing single-key installations keep working across restart, including empty-key legacy behavior until explicitly changed.
...
- [x] Synthetic multi-key integration covers non-stream/SSE/first-event retry/same-session collision, six scheduler modes and scoped cache/low-role/growth pools, empty and blocked pools, in-flight reassignment/rotation/revocation and both model auth header forms; each relevant mock asserts no cross-owner upstream attempt. Not an exhaustive 24-order × all-race matrix.
- [x] Legacy config/env precedence, successful/malformed migration, old-client account/security save, reboot and key reassignment/rotation/revoke keep config bytes safe and stable account IDs; no orphaned accounts or dangling bindings. Synthetic old-image rollback demonstrates an **unsafe** direct downgrade and the required full pre-upgrade DATA_DIR restore gate.
- [x] Auth/CSRF/role, ordinary/detail logging and credential-redaction tests prove new key cannot manage, cannot appear in diagnostics, and client credentials never become upstream Authorization. Browser draft/keyboard/confirmation tests exercise management UI with only temporary local mock data.
