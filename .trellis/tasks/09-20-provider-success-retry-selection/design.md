...
Provider cooldown/quarantine保证该provider不再进入本次/未来候选；account cooldown/quarantine结束账号chain并遵守最多一次replacement。degrade/ignore只写健康facts，不决定retry。下一账号重新建立其route plan，但Provider-model hard state按共享model/provider继续生效。
...
逐次selector可回退到原planner，但不得恢复multi-provider order。新Provider state/health由前置子任务所有，不在本任务迁移。
