...
`DETAIL_CONFIRMED`, `ERROR_DETAIL_CONFIRMED`, `RAW_BODY_CONFIRMED`, `DETAIL_TOGGLE_PENDING` and `DETAIL_SETTINGS_ID` own full/error-only and independently default-off raw-body settings. A stale settings GET cannot undo a later POST; a failed POST restores all three confirmed modes. Each toggle posts only its changed field, while the server response refreshes all three confirmed values. Navigation away suppresses stale status messages, but a completed setting write still updates the confirmed values. Do not call `loadAll()`, `saveAccounts()` or mutate ACCS, bulk selection, raw editor or live scheduling controls for any diagnostic action.
