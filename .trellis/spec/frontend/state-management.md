...
```text
GET /api/accounts
  -> { accounts: [{ ..., clientKeyId, health, cachePoolRole, cachePoolQuotaRole,
...
GET /api/security/client-keys -> { keys: [{ id, name }] } // Legacy plus extras; no key
POST /api/security/client-keys <- { name } -> { id, name, key } // once
PATCH /api/security/client-keys/<id> <- { name } -> { id, name }
POST /api/security/client-keys/<id>/rotate <- {} -> { id, key } // once
DELETE /api/security/client-keys/<id> -> { ok: true } // 409 while accounts remain attached
...
```js
{
  id, clientKeyId, name, note, key, enabled,
...
#### Client-key list, one-time secret and account owner draft

`loadAll()` accepts `/api/security/client-keys.keys` as ID/name labels beside the complete `ACCS` snapshot; `renderClientKeys()` and `clientKeyOptions()` escape names/IDs and show account counts from the **current draft**, not a promise about saved attachments. `#newAccountOwner` and drawer `#drawerClientKeyId` select valid IDs; changing the drawer selector does not call the server until the separate explicit `saveAccounts()` full-list write. Search, row redraw, bulk concurrency, scheduling presets and the raw scheduling editor never reconstruct owners from visible rows. Saving accepted accounts reloads `ACCS` and the key list. The single `cachePoolTargetSize` is applied independently to each owner's pool; `ACCS.cachePool.scope === 'per-client-key'` and `actual` is an aggregate of derived active roles, not a per-key quota.

`createClientKey()` posts only `{name}`; `renameClientKey()` patches only `{name}`. `rotateClientKey()` posts `{}` and `revokeClientKey()` sends DELETE only after native confirmation; a cancelled dialog sends no write. `clientKeyWrite()` clears any previously displayed secret, performs one admin `api()` write, immediately reveals a successful create/rotate response secret while the action generation is current, and then reloads accepted server state; a `409` for attached accounts leaves the key visible and reports reassignment required. If the post-write refresh fails without a 401, keep the revealed secret available for manual copy and say the operation already took effect, the list failed to refresh, and the write must not be repeated; rename/revoke likewise require a refresh/recheck rather than being called failed. A 401 or navigation invalidates the action generation, clears the secret and cannot revive it; ordinary dialog dismissal clears the value without cancelling the accepted list refresh. Key operations can replace an unsaved account draft through `loadAll()`; tell the operator to save account edits first. `showClientKeySecret()` holds only the new generated `key` from that **one** create/rotation response in the readonly native modal, not in `CLIENT_KEYS`, `ACCS`, URL, localStorage or ordinary status text. `closeClientKeySecret()` clears the input and copy status on dismissal, navigation, 401/session loss and before another write; Escape routes through the same close owner. Only an ordinary user dismissal returns focus to the opener; navigation, pagehide and session loss close without focusing the old console control. If dismissal precedes `loadAll()` replacing the row opener, resolve a callback to the current DOM button on close and the same-action completion may restore focus only when the browser has fallen back to `document.body`, a disconnected node or the closed dialog while still in the console; navigation/401 or user focus on another control forbids refocus. Clipboard denial selects the field for manual copy with live feedback. A stale action cannot reveal a secret after a later close/navigation; successful writes refresh labels from the server, not the returned secret. The Legacy password input is an explicit admin-only compatibility exception, not an extra-key list.

#### Rendering and sensitive values

All server-provided text used in HTML strings passes through `escapeHtml()` (or `jsArg()` where a JavaScript string argument is needed). Provider health renders `degraded`, active cooldown remaining time, and expired/half-open state without reordering the stable provider list. Status regions use `aria-live="polite"`. Account keys are password inputs unless the operator explicitly enables “show keys.” This is display protection only; account data comes from the independently authenticated administrator API (Cookie, transport/Origin and CSRF for writes), **not** from the client proxy key.
...
Raw sessions and message content must never be added to account, model, history or ordinary diagnostic state/views. Account notes, proxy URLs/authentication, account Keys and custom Header values remain owned by the authenticated drawer and must not be copied into ordinary log rows/details. Only the separately enabled detailed panel may hold bounded captured model HTTP content under the independent administrator contract in `../backend/logging-guidelines.md`. Legacy details are sanitized; a separately explicit raw profile contains unredacted bodies that may include arbitrary credentials. Raw mode exposes only fixed safe structural Header values and constant-redacted Authorization/Cookie/Set-Cookie names on the selected detail (never arbitrary/Header credential values or list metadata); never copy bodies into account drafts or browser storage.
...
The read-only `modelProvidersPanel` sits immediately after detailed logs in the top navigation. `MODEL_PROVIDER_QUERY_ID`, `MODEL_PROVIDER_CONTROLLER`, and `MODEL_PROVIDER_DATA` own only this feature's last accepted authenticated `/api/statistics` snapshot, in-flight query and filter projection. Navigation/pagehide aborts and increments its generation; pageshow restarts a read only while this panel is visible, so an aborted back-forward-cache visit is not stranded. Stale success, catch and finally cannot render a hidden or newer visit. Filter/redraw reuses the accepted snapshot without rehydrating `ACCS`, raw rules, routes or any other draft. The labelled search input, native refresh button and two focusable horizontally scrollable table regions support narrow widths and keyboard access. Native child buttons select model (default) or channel projection with `aria-pressed`/`aria-controls`; selection and search redraw only the selected table from `MODEL_PROVIDER_DATA` and remove inactive rows from the DOM without fetching, changing account drafts or resetting focus. The live status announces the selected projection; top-level entry still focuses the heading. Model rows show `请求成功率` from final non-cancelled requests and named channel rows show `渠道尝试成功率` from real attempts as a percentage plus sample count; zero samples say `无数据 · 0 样本`. Unknown Provider has no invented attempt-success rate. Model final request success excludes cancellation; Provider health success counts named real attempts only. The model aggregate may include pre-v5 usage, while v5 Provider usage/valuation show separate tracking-start/loss coverage, explicit unknown Provider, known-count denominators, no-data, and USD reference snapshot metadata. Historical v1 frozen amounts retain their four exact IDs and integer-thousandths tariff display; current v2 lists 12 exact IDs and ten-thousandths input/output/cache-read/cache-write rates with per-model official source, project collection/version, unknown official effective date and labelled DeepSeek peak/off-peak range. The v2 current tariff table is displayed even when no request can be valued; priced Cached Write and unknown Qwen context bands are explicitly non-computable. Default model rows omit channel rows and show compact input/output, cached-read/Token ratio, and reference consumption-equivalent USD; channel rows retain the model ID and show only final-success-attributed usage, not failed retry usage. Price amounts are compact and explicitly labelled partial if eligible/priced success counts, model/final-request coverage or valuation coverage do not match; expanded native row details retain exact known-request counts, total, cache-hit denominator, independent coverage start/loss, frozen version/rates, DeepSeek interval and unpriced reasons. The native help disclosure contains formulas, sources, migration and non-billing caveats; keep the current tariff table separately expandable. Eligible priced counts are compared against final success counts; no value is rendered as a subscription bill or deducted from the separate $50 monthly reference remaining estimate. Escape model/Provider/version text before `innerHTML`; values that are not covered or overflowed remain unavailable, not zero.
...
`DETAIL_CONFIRMED`, `ERROR_DETAIL_CONFIRMED`, `RAW_BODY_CONFIRMED`, `DETAIL_TOGGLE_PENDING` and `DETAIL_SETTINGS_ID` own full/error-only and independently default-off raw-body settings. Every off-to-on raw change requires a native confirmation **before** POST; cancellation restores the confirmed checkbox without writing, including after a prior disable. A stale settings GET cannot undo a later POST; a failed POST restores all three confirmed modes. Each toggle posts only its changed field, while the server response refreshes all three confirmed values. Navigation away suppresses stale status messages, but a completed setting write still updates the confirmed values. Do not call `loadAll()`, `saveAccounts()` or mutate ACCS, bulk selection, raw editor or live scheduling controls for any diagnostic action.
...
`DETAIL_BODY_TEXT` is null during reads/invalidations; `DETAIL_BODY_RAW` labels the selected manifest's raw profile. Copy uses only its loaded authenticated body string, not metadata, network errors or stale text. Loaded raw text is cleared on selection, navigation, logout, session loss and pagehide. Clear captures navigation before awaiting DELETE, invalidates pending detail reads, removes loaded content and reloads only if that same details navigation is still visible. Missing/expired bodies remain a safe empty/error state.
...
| Existing account ID is unknown/changed or duplicated, or `clientKeyId` is unknown | server `400`; do not overwrite the accepted `ACCS` snapshot |
| Additional-key revoke while any saved account still owns it | server `409`; keep labels/owner assignment, require a separate saved reassignment first |
| Legacy is blank with Legacy-owned accounts and an extra-key create is requested | server `400`; preserve anonymous Legacy model traffic until explicitly secured |
...
- posting `/api/accounts` with the full account snapshot preserves stable IDs, `clientKeyId`, `perModel`, notes, proxy/Header fields, weight/priority, mode, active account, wait, and rules; a filtered/drawer/bulk/preset/raw edit retains each hidden owner's ID, and old saves omitting owner preserve stable-ID ownership server-side; `test/account-draft.test.js` exercises production drafts;
- key list escaping, native confirm cancellation, one-time create/rotate secret, copy fallback, modal dismissal/401 clearing, attached-key revoke conflict and accepted-state reload run in `test/account-draft.test.js`, `test/admin-ui.test.js`, `test/ui-contract.test.js`; real-browser focus/keyboard/narrow scrolling evidence is separate;
