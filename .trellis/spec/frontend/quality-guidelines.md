...
Filtering by name/note changes rendering only; it must not change account indexes, active-account identity, owner `clientKeyId`, or the submitted full list. `#newAccountOwner` and the labelled drawer `#drawerClientKeyId` select an ID from the accepted key list; changing owner is only a draft until the full account save. The account table shows escaped owner labels, not credential values.

#### Client-key security controls

The security panel lists only the accepted key `{ id, name }` labels and counts based on current account drafts, including the fixed Legacy row; escape label/ID text before `innerHTML` and use `jsArg()` for action arguments. Native Create/Rename/Rotate/Revoke controls use the existing authenticated `api()` (Cookie + CSRF for writes), not client Bearer storage. Rotation and revocation require native `confirm()` before any request; cancelling changes nothing. Revocation with saved accounts returns `409` and instructs a separate account reassignment/save first. After a successful create/rotate response, reveal the one-time key immediately before reloading accepted server state; a failed non-401 refresh must retain the revealed key/manual selection and announce that the write took effect rather than invite a duplicate write. A key operation may discard unsaved account edits, so warn first. `PROXY_KEY` guidance says a nonempty environment value overrides Legacy only at startup/restart; a running admin save can change it until the next restart.

Only the create/rotate response contains the generated secret. Show it once in `#clientKeySecretDialog.showModal()` in readonly `#clientKeySecret` and focus it; never render it in the label table, normal list, status, localStorage or URL. Copy uses `navigator.clipboard.writeText` with a selected-input/manual-copy fallback and live feedback. Closing via button/Escape, navigation, pagehide or session loss empties the input/status. Ordinary dismissal returns focus to the opener; if an accepted list reload subsequently replaces that row button and focus falls to the document body or remains in the closed dialog, restore focus only to its new same-operation opener. Navigation/401/pagehide closes without returning focus to the old opener, invalidates that operation and must never steal focus or revive a secret. The key table wrapper is labelled and focusable with native horizontal scrolling at narrow width. Browser smoke (synthetic local Chromium at 390px) verified Enter-based create and drawer entry, modal focus/Escape return, clipboard-denial fallback, native rotate/revoke confirmation and ArrowRight horizontal scroll; it did **not** verify screen-reader announcements or a production ingress. VM/static assertions alone do not prove focus or responsive behavior.
...
The top navigation exposes six mutually exclusive sections: console, statistics, request logs, error logs, detailed logs, and model/Provider statistics. The active native button uses `aria-pressed="true"`; the other buttons are false. Selecting statistics or a log section hides the complete console panel. Do not implement navigation with anchors, `scrollIntoView()`, or duplicate section DOM.
...
#### Read-only model/Provider statistics panel

The sixth button appears immediately to the right of detailed logs. The page does not trigger a quota refresh or write any configuration. It reads the authenticated statistics projection on entry and manual refresh, projects local model/Provider filter changes without an API call, and invalidates aborted/stale page reads independently of other sections. A visible panel rereads on `pageshow` after `pagehide` abort; a hidden panel does not. Keep a native labelled search/refresh, heading focus entry, live status, and focusable horizontal table regions on narrow screens. Native pressed child buttons select a default model table or channel table without a fetch; search filters only model ID in model view and channel/model ID in channel view. The inactive table has no retained rows: reselecting it reconstructs rows from the accepted snapshot without a fetch. Both retain the accepted statistics snapshot, focused button, and account drafts. Distinguish final request success (excluding cancelled) from named attempt health success using separate headers `请求成功率` and `渠道尝试成功率` with percentage and sample count (`无数据 · 0 样本` when empty); explicit unknown Provider is not an attempt success rate. Compact rows show input/output Token, cached-read and cache Token ratio, and USD reference consumption-equivalent (partial when priced eligibility, final request coverage, model Token coverage or valuation coverage is incomplete); native per-row details expose Token field-specific known-request counts, cache pair ratio versus request-hit denominator, reference valuation priced/eligible coverage, versioned historical tariffs and migration/cell-loss status. The help disclosure retains the denominator/formula/source explanations; the separate tariff table remains expandable. Channel amounts and usage belong only to attributed final successes, not failed attempts. A known zero is numeric zero; absent or invalid usage/price and overflow are not zero. The page labels amounts “参考消费等值（USD），非订阅实际扣费” and lists the current 12 v2 model tariffs even without priced usage, with per-row input/output/cached-read/cached-write, source, tier, project collection/version and unknown official effective date; previous v1 amounts retain their own unit scale/version. The DeepSeek direct official source supplies both peak/off-peak rows (ClinePass lists only V4.1 Flash peak), but the page always displays a reference range rather than an applied tier. Qwen Cached Write/context bands without matching upstream evidence remain unpriced, not zero; partial eligible requests and overflow are explicit. This is independent of the account community $50 monthly remaining estimate. `test/model-provider-ui.test.js` runs the production script for safe escaping, range/no-data, filter/draft preservation and stale reads; it is not browser evidence.

#### Detailed log panel

`detailsPanel` has its own labelled fifth navigation button and never reuses ordinary `logPanel` state. Show persistent default-off/content/privacy warnings, explain error-only versus full capture and full-mode precedence, label sanitized 5 MiB/7 days and explicit raw 35 MiB/48h limits with shared 512 MiB retained reservation (not RSS) and 1 GiB disk, and show that independent administrator authentication is always required even when the client proxy key is empty. All three checkboxes (full, error-only, raw bodies) are disabled until settings load and while a settings POST is pending. `rawBodyAvailable:false` leaves raw opt-in disabled with a persistent text-only availability explanation, while full/error sanitized controls remain usable; a preexisting raw=true can still be turned off. Failure restores all three last confirmed modes without touching account drafts.

The list shows metadata only, including profile, HTTP/outcome/result and capture state. `selectDetail(requestId)` loads sanitized legacy Headers or fixed safe raw Header names/structural values (with constant-redacted Authorization/Cookie/Set-Cookie), attempt metadata and body descriptors into `textContent`; `loadDetailBody()` loads one sanitized legacy or explicitly unredacted raw `text/plain` body into a labelled readonly textarea. Raw warnings must explicitly say the body may include passwords/keys and private conversations, while selected raw detail Headers are only safe projections. Every off-to-on raw checkbox action requires native risk confirmation before its settings POST; cancellation is a no-write rollback of the checkbox. Clear loaded raw text on selection, navigation, logout, pagehide and session loss. Body buttons identify client/attempt request/response, completeness and captured/observed byte counts. Do not automatically load every body on a page. Use escaping/`jsArg()` for button/table HTML and text nodes or textarea `.value` for arbitrary captured content.
...
Copy uses only `DETAIL_BODY_TEXT` obtained from the authenticated body API; warn when copying raw text, is disabled until loading completes, announces failure and focuses/selects the readonly text as fallback. Clear explicitly names detailed logs and preserves ordinary logs. Filter edits invalidate stale results/cursors and ask for refresh; Next stays disabled during pending/invalidated queries. Draft/navigation ownership is in `state-management.md`.
...
| Extra-key rotation/revocation confirmation is cancelled | no management write and no secret reveal |
| Revoke a key while saved accounts still own it | display server `409`, preserve key list and accepted ownership until separate saved reassignment |
| Clipboard API denies a one-time key copy | select the readonly input and announce manual copy; do not persist it in browser storage |
| Navigation, pagehide, 401 or secret-dialog Escape | clear transient generated secret/copy status; return focus to the opener only on ordinary Escape dismissal, not navigation/pagehide/401 |
...
- account snapshot preservation for hidden fields including `clientKeyId`, canonical `errorRules`, all three pipeline booleans, the three-step order, and every bounded pipeline integer (`cachePoolSize`, `cachePoolMaxSize`, `cachePoolLowQuotaSize`, both TTLs, `sessionBindingMaxEntries`);
...
- six mutually exclusive top sections with one statistics panel, one shared ordinary-log DOM, independent detail and model/Provider statistics panels, explicit active state, and no anchor/scroll shortcut;
