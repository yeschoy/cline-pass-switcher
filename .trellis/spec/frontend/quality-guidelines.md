...
`detailsPanel` has its own labelled fifth navigation button and never reuses ordinary `logPanel` state. Show persistent default-off/content/privacy warnings, explain error-only versus full capture and full-mode precedence, label sanitized 5 MiB/7 days and explicit raw 35 MiB/48h limits with shared 512 MiB retained reservation (not RSS) and 1 GiB disk, and show that independent administrator authentication is always required even when the client proxy key is empty. All three checkboxes (full, error-only, raw bodies) are disabled until settings load and while a settings POST is pending. Failure restores all three last confirmed modes without touching account drafts.
