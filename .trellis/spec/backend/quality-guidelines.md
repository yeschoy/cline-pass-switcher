...
- `accountPipeline` has exactly four booleans plus `order`, an exact permutation of `excludeUnhealthy`, `quotaPool`, `healthSort`, and `sticky`. When all booleans are false, `acquireAccountLease()` calls the unchanged legacy selector regardless of order. Enabled pipelines first apply mandatory account eligibility, then each enabled step in stored order. Each step stably refines existing candidate groups, so later stages cannot cross earlier priority groups. Unhealthy all-candidate fallback is restricted to the earliest prior group; quota-all-unknown remains a no-op. Explicit sticky splits each current group into HRW-ranked singletons; sticky account mode injects it once at the end only when the sticky flag is off. Capacity fallback cannot bypass eligibility, and provider retries remain bound to the selected account.
...
| An older client sends all four pipeline booleans but omits `order` | preserve the current server order |
| `/api/accounts.accountPipeline` is non-object, incomplete, has unknown keys/non-booleans, or has a non-permutation `order` | `400`; do not save |
...
- all-false pipeline output is equivalent to each legacy mode, while every stored order round-trips and representative quota/health/sticky permutations prove earlier-stage priority, implicit sticky compatibility, eligibility, capacity fallback and lease release;
