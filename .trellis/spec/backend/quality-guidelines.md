...
- Frozen ClinePass reference snapshot `clinepass-2026-09-24-v1`, collected `2026-09-24`, source `https://docs.cline.bot/getting-started/clinepass`, currency USD, official effective date **unknown**. Only `cline-pass/kimi-k3`, `cline-pass/glm-5.3`, `cline-pass/deepseek-v4-flash`, and `cline-pass/deepseek-v4-pro` are priced. DeepSeek peak/off-peak is a **range**, not an inferred applied tier. The official per-million input/output/read rates are in `server.js` and the task's `research/official-pricing-and-usage.md`; this is a **reference usage equivalent, not an actual bill** or a community $10/$25/$50 quota. Cache-write-priced models are unpriced without explicit write counts. Valuation requires explicit safe nonnegative input/output/cached-read with `cachedRead <= input`; fixed-point BigInt picodollars calculate `(input-cachedRead)*inputRate + cachedRead*readRate + output*outputRate`, with separate overflow markers and eligible-request coverage. Missing price/usage/inconsistent counts cannot produce $0. Each request freezes amount and snapshot version inside the existing bounded minute owner; historical cells are never repriced. Version/cell eviction marks incomplete coverage, not zero.
