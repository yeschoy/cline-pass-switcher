...
New private deployment.json record (older record shapes remain readable):
  { release, commit, image, previousRelease, previousImage,
...
- Do not assume the host has Node.js or another helper runtime merely because the application container does. Before any mutation, verify each host-side helper used by preflight; prefer existing `python3` for bounded JSON projections or run Node only inside the known application image/container. The production Compose and operator files are root-private: read only with verified noninteractive `sudo -n`, not by relaxing file permissions. A missing helper must stop or fall back during read-only preflight, never after the compose switch.
- Pass release names, hashes and paths to remote helpers as validated positional arguments or fixed environment values. Do not interpolate them through nested local/SSH shell quoting. A quoting failure is retryable only while no release directory, backup or compose mutation exists; record the retry in deployment evidence. `docker compose config --format json` may project a byte limit such as `mem_limit` as a canonical decimal **string**, not a JSON number; validate the parsed value and actual `docker inspect HostConfig.Memory` instead of failing a type-only equality check or weakening the memory gate.
...
- A release that intentionally normalizes a new persisted configuration field may use a predicted post-migration hash only after running the committed image against byte-for-byte config/metadata copies. The copied result must differ solely by the documented non-secret schema projection, and its SHA-256 becomes the immediate post-switch gate. Preserve the original bytes for rollback. **Pre-switch hash drift aborts the switch** until a fresh copy is rehearsed. If the hash changes *after* that gate while the new image remains healthy, do not automatically overwrite possible operator saves: stop final acceptance, compare only safe changed field paths/allowed projections against the rehearsed copy, preserve both current bytes and private backup, obtain non-secret operator intent, then either accept a newly confirmed hash or perform a separately fenced rollback. Never attribute a later operator save to startup migration or declare a pending release complete. This exception never permits uploading a local config or printing secrets.
- Before switching, copy `compose.yml`, `deployment.json`, `data/config.json`, `data/metadata.json`, and the independent `data/admin-auth.json` when present into private versioned backup/verification paths. Raw detailed groups (`data/detailed-logs/raw/`) must **not** enter the ordinary release backups. Before any separately authorized production raw enablement, verify exclusion or a separately enforced private 48-hour backup expiry on every copied-data path; rehearse that exact backup and rollback combination on isolated copies. A service TTL cannot expire external copies. Missing admin state on a legacy release requires an explicit one-time bootstrap plan with independent private code and effective client-key seed (or separate nonempty seed when the client key is empty). Configure the public HTTPS origin and a private 64-character lowercase-hex `CLINE_PASS_ADMIN_PROXY_TOKEN` shared only with the TLS reverse proxy (which replaces the incoming proxy-attestation Header), rehearse the first password change and management-script Cookie/CSRF migration against a private data copy before live switch; never log the code, password, proxy token, cookie, CSRF token or verifier. Metadata may legitimately change while the service runs; retain its backup and report both hashes.
...
- Atomically install the already-built candidate Compose and switch with `docker compose ... up -d --no-build`. Require `cline-pass-console` to be `running`, `healthy`, on the captured image ID, with zero restarts. Never rebuild during the switch because Compose provenance labels can produce a different image identity from a prior direct build. Once these gates pass, **the new code is deployed** even if administrator acceptance remains pending; report the running commit/image/health plainly and keep `deployment.json.status=awaiting-admin-acceptance` until the remaining authenticated gates pass. A pending acceptance record is not evidence that the old code is still running. Never call a protected endpoint's 401 response an authenticated success.
- Roll back the compose file and restore the previous healthy container with `up -d --no-build` when build, startup, health, local endpoint, authenticated management API, internal network alias, or immediate copy-predicted config-hash validation fails. For later unexpected operator-data drift, apply the read-only confirmation/fenced recovery rule above; do not overwrite possibly intentional post-switch saves as an automatic side effect. For an auth-migration rollback, retain and restore the private admin-state backup when reapplying the new release. The old image's shared client-key management bypass returns during rollback: isolate management ingress and keep detailed content capture off until the independent-login release is restored; never claim the old image preserves the new boundary. A raw-body release additionally requires verifying that the old image cannot traverse the private raw child and rejects `raw-full`/`raw-error` manifests even if under the old body cap; do not expose its management endpoint during rollback. Never roll back to a shared-key image with management ingress open while raw groups or backups exist. When a declared config migration was applied and the current hash still equals the copy-predicted migration hash, rollback atomically restores the original backed-up config before starting the previous image. If later operator writes changed the hash, preserve those bytes and obtain a fenced recovery decision rather than overwriting them.
...
- Roll back by setting `CLINE_PASS_SSE_HEARTBEAT_MS=0` to stop comments, or use the normal committed-HEAD rollback path for the full connection-pool change. Neither switcher nor this deployment procedure offers inbound HTTP/2/h2c, `/v1/realtime` WebSocket, or `/v1/responses` routing. Local integration tests alone do not prove paid/live New API behavior. The later `32a150e` production release exercised an isolated Node 22-alpine candidate and machine health gates, but no paid chat or end-to-end New API SSE; Node 18 runtime execution remains unverified.
...
| Pre-switch config hash differs from the copied baseline | Stop before switching; preserve live bytes and rehearse a fresh copy |
| Immediate post-switch config hash differs from the copy-predicted migration hash | Fence the observed bytes and roll back when safe; do not overwrite a concurrent unknown operator save without confirming intent |
| Healthy switched image, but independent administrator read is not yet confirmed | State clearly that the new code **is deployed**, record `awaiting-admin-acceptance`, preserve rollback; do not mark `completed` or treat a 401 as an authenticated read |
| A later config save changes a safe projected path after initial hash gate | Preserve current bytes, mark acceptance pending, ask whether it was intentional; accept a new hash only with operator confirmation or perform a fenced rollback |
...
- **Bad:** treat a client-key-only 401 as proof the administrator can read management pages, overwrite a later operator config save with the deployment backup, or answer “not deployed” when the new image is already healthy.
...
```text
Wrong: image is healthy but admin checks are pending → report "not deployed";
       later config hash differs → blindly restore the backup.
