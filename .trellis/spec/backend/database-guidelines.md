...
PROXY_KEY       optional nonempty startup/restart override of effective Legacy proxyKey; in-process admin save can change it until next restart
...
```js
{
  ...
  proxyKey,            // private stored Legacy secret; stable client owner ID 'legacy'
  clientKeys,          // private bounded additional [{ id, name, key }] only; see client-key-guidelines.md
  publicBaseUrl,
  exposeCatalog,
  upstreamBase,
  knownModels,
  accounts: [{
    id, clientKeyId, name, note, key, enabled, // clientKeyId is downstream owner ID; key is upstream secret
...
Account `id` is the stable join key for runtime state; names are editable and keys are not identifiers. `clientKeyId` is an exclusive downstream owner ID, validated against `legacy` plus `clientKeys`; `key` is intentionally durable **upstream** static configuration. The seven-section inventory/migration and rollback contract is in [client-key-guidelines.md](./client-key-guidelines.md). Do not copy either client or upstream key material into metadata, history, status reasons, or routing diagnostics.
...
Startup normalization is permissive for absent legacy fields, not for malformed canonical `clientKeys` or unknown explicit owners; these fail startup without rewriting the original bytes. Management APIs validate strictly before normalization and persistence. Old account saves omitting an owner preserve an existing stable ID's owner; additional-key deletion with attached accounts fails `409`. See [client-key-guidelines.md](./client-key-guidelines.md).
