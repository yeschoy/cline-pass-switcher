...
GET  /api/logs/settings
  -> { detailedLogging, errorDetailLogging, rawBodyLogging, authRequired, maxBodyBytes, rawMaxBodyBytes, maxPayloadBytes, maxAgeMs, rawMaxAgeMs, maxTotalBytes, health: { failures, dropped, corrupt, lastFailure, rawWarnings, dropReasons } }
...
- The complete fixed `DETAIL_DROP_REASONS` key set (all nonnegative safe integers, including zero buckets) in declaration order is:
  ...
  | `captureBudget` | Shared 512 MiB retained raw or expanded sanitized/encoded payload reservation fails |
