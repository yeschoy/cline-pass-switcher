...
Ordinary request/error logs are diagnostic projections, not raw request dumps. A proxy response must not fail because diagnostic persistence fails, and these logs must never become a second storage location for credentials, prompts, sessions, proxy URLs, account notes, or upstream response bodies. The separately authenticated, opt-in detailed store below is the only approved body exception (sanitized legacy headers remain available); it does not change this ordinary-log projection or metadata exclusions.
...
All log APIs use the independent initialized administrator Cookie/CSRF and same-origin transport boundary, not a client/proxy key. Query `limit` is 1-200 and pagination uses the opaque `nextCursor` returned by the previous page.
...
Apply when changing `lib/detailed-log-capture.js`, `lib/detailed-log-store.js`, detailed API routes or their transport hooks. Only this default-off store may retain sanitized legacy headers and prompt/response content; explicitly opted-in raw profiles may retain unredacted bodies and only independently parsed, bounded safe Header projections, never arbitrary/raw Header values. Never copy detailed bodies into ordinary JSONL, metadata, service errors or diagnostic headers.
...
GET  /api/logs/settings
  -> { detailedLogging, errorDetailLogging, rawBodyLogging, rawBodyAvailable, authRequired, maxBodyBytes, rawMaxBodyBytes, maxPayloadBytes, maxSanitizedPayloadBytes, maxAgeMs, rawMaxAgeMs, maxTotalBytes, health: { failures, dropped, corrupt, lastFailure, rawWarnings, dropReasons } }
POST /api/logs/settings
  <- a non-empty exact subset of { detailedLogging: boolean, errorDetailLogging: boolean, rawBodyLogging: boolean }
  -> { ok: true, detailedLogging, errorDetailLogging, rawBodyLogging }
...
GET  /api/logs/details/<requestId>/bodies/<bodyId>
  -> UTF-8 text/plain (sanitized legacy or explicitly enabled raw profile)
...
- All settings/detail APIs require the independent administrator Cookie session even when the downstream client key is empty; no proxy key or legacy Header grants management access. Both full and error-only detailed capture are suspended while administrator state is absent/pending first password change, even if an older config persisted a switch on; ordinary model traffic remains unaffected. Unsafe methods require CSRF and same-origin checks. UI must never claim that empty client-key mode leaves detailed content unprotected. Settings/detail responses use `Cache-Control: no-store`; bodies additionally use `X-Content-Type-Options: nosniff`. IDs must be validated and body IDs must belong to the selected manifest; reject traversal/symlink paths. Read-versus-clear/eviction is a safe missing response.
...
- The complete fixed `DETAIL_DROP_REASONS` key set (all nonnegative safe integers, including zero buckets) in declaration order is:
  ...
  | `captureBudget` | Shared 512 MiB or sanitized-subset 64 MiB retained payload reservation fails |
...
- Authenticated `GET /api/logs/settings` and `GET /api/logs/details` expose the same additive aggregate `health.dropReasons` object; older clients may ignore it. No per-request reasons or new reason field belongs in ordinary request/error JSONL, `metadata.json`, detailed manifests/descriptors, error responses or service logs; no body, Header value, credential, raw session or request identity may enter the counters or labels. Clear still advances its publication generation, not a persisted reason history. The existing sanitized modes keep their 5 MiB body and seven-day retention limits; the shared reservation is 512 MiB, the sanitized subset remains capped at 64 MiB, and raw capture is independently default-off and runtime-gated. Diagnostic failures remain fail-open. Do not expose raw errors, paths, bodies or credentials in service output.

#### Explicit raw-body profile (security exception)

- `rawBodyLogging` is an independent persisted boolean, default `false`, accepted by strict persist-first detail settings writes **only when** `rawBodyAvailable` is true. Readiness requires explicit process-only `CLINE_PASS_RAW_BODY_READY=1` and a detected cgroup/host memory limit of at least 2 GiB; Linux with unknown cgroup limit fails closed. The minimum is not a performance proof, and production activation additionally needs private-backup, rollback and target-load acceptance. A rejected POST true returns safe 409 without changing config/runtime bytes; a persisted true on an unready runtime suppresses detailed capture rather than writing raw or silently relabeling it sanitized. GET settings exposes `rawBodyAvailable` and the 64 MiB `maxSanitizedPayloadBytes`, while `maxPayloadBytes` remains the 512 MiB shared maximum. It changes only future full/error captures when one of those switches is enabled and administrator initialization has completed. An old `detailedLogging: true` does not opt into raw. Raw captures are `raw-full`/`raw-error` (never represented as old `full`/`error`), with metadata-only summaries and `redacted:false` descriptors. Old `full`/`error` groups remain readable with the original 5 MiB/7-day semantics. Old validators reject raw manifest profiles, even if a raw body is smaller than 5 MiB.
- Raw mode does not instantiate `DetailRedactor` or snapshot configured credentials. Each client ingress/upstream attempt input/output/final downstream body captures exact valid UTF-8 up to 35 MiB, truncating only the diagnostic copy; invalid UTF-8 is explicitly omitted. Error-only raw captures request and response bodies of failed attempts only and releases successful attempt inputs. It never promotes body data to ordinary JSONL, metadata, service errors or model responses. A raw body itself may contain ANY credential; the admin sees it by explicit exception. No arbitrary Header values or free-form account/model/provider/URL fields enter raw manifests/list summaries. Preserve a bounded fixed-field metadata projection, only safe enums/UUIDs/statuses and descriptors.
- `projectRawHeaders()` runs at the native ingress/attempt/downstream boundary, retaining no unprojected Header map in a raw root. Only `content-type`, `content-length`, `accept`, `content-encoding`, `transfer-encoding`, `authorization`, `cookie`, `set-cookie` names are retained (at most eight). The last three fixed names have constant `[REDACTED]` values, regardless of input; other names, including attacker-chosen credential-like, custom and URL-bearing names, are omitted. Exactly `application/json`, `text/event-stream`, `text/plain` media values survive; `content-type` may parse their exact `; charset=utf-8` suffix to the canonical media value. Decimal length is canonical 0 or 1–9 digits without leading zero; content encoding is `gzip`/`br`/`deflate`/`identity`, transfer encoding is `chunked`/`identity`. Unknown, oversized (over 32 characters), array or duplicate values become `[REDACTED]`; over 64 source keys or 128 native pairs omit the whole map. These deliberately lossy structural grammars do not prove the absence of arbitrary secrets in raw **bodies** and must not be expanded to custom values by generic text redaction. Raw full stores ingress and each attempt request/response plus downstream Header maps; raw error stores only failed-attempt request/response maps, never ingress/success/downstream maps. Maps are optional in previously published raw manifests; `DetailedLogStore.validate()` enforces exact names, canonical values and bounds on publication and detail/body reads. List/inventory summaries never include maps.
- The one `DetailedLogStore` owns sanitized UUID groups in its legacy root and raw groups in a private `raw/` child. Raw group names are strict immutable canonical timestamp + UUID; only allowlisted regular manifest/UUID-body files and no symlinks prove ownership independently of manifest parsing. A readable manifest must match the directory timestamp. Reads reject raw groups at 48 hours; startup and minute maintenance delete demonstrably owned expired groups even if the manifest is corrupt. Physical deletion can lag the deadline until the next sweep or a failed-storage retry, and is not an exact wall-clock erasure guarantee. Suspicious/unknown entries remain inaccessible and undeleted and produce a bounded `health.rawWarnings` count for manual review; an unknown entry conservatively blocks unprovable capacity admission. Tracked raw temporary groups follow the same ownership check. Both roots share the serial queue, clear generation, 1 GiB disk admission, bounded inventory and oldest-group eviction. The 512 MiB capture reservation is not an RSS cap, and at most four admitted raw API body reads/sends can each load up to 35 MiB on demand; admission occurs before metadata queueing and is released only after the HTTP response finishes or closes.
- Independent admin session, transport/Origin and CSRF guard covers settings/list/detail/body; neither client key nor an old image is authorized to view raw content. Body reads are `no-store`/`nosniff`; browser does not fetch every body eagerly, renders body text only in a readonly textarea, warns explicitly about raw credentials and discards loaded content on selection, navigation, logout and session loss. Preserve a leading UTF-8 BOM with `TextDecoder('utf-8', { fatal: true, ignoreBOM: true })` in both capture and retrieval; the decoder default silently strips it, violating byte-faithful raw text. Service TTL cannot delete external backups; before separately authorized production enablement, exclude `raw/` from backup or apply an independently enforced private 48h backup expiry and rehearse old-image management ingress isolation and raw-reader denial on a private copy. This task does not deploy or enable raw mode in production.
...
| POST enables raw without explicit runtime flag and detected ≥2 GiB memory | 409; no persistence/runtime change; sanitized mode stays available, while preexisting raw=true does not capture on the unready runtime |
...
`test/raw-detail.test.js` must assert the one shared budget's 64 MiB sanitized-subset fence and 512 MiB raw ceiling, explicit default-off mode, admin-only on-demand reads, Header/list/ordinary isolation, exact valid UTF-8 including an initial BOM across restart, invalid-byte omission, raw error-only failed-attempt selection/SSE event bytes, shared capacity, four held raw reads, owned corrupt-manifest deletion at 48h, unknown/symlink preservation plus warning, deletion retry and legacy root compatibility. Browser VM tests must clear loaded raw text on navigation, logout, session loss and stale response; the actual target-container memory envelope, real-browser controls and production backup/rollback rehearsal remain separate enablement gates.
...
Run module/server syntax checks, embedded browser-script compilation and `env -u CLINE_PASS_KEY -u PROXY_KEY -u PUBLIC_BASE_URL -u PORT npm test`. Before deployment, synthetic tests under the target container memory limit must show that a high count of sanitized 5 MiB captures drops diagnostics at its 64 MiB subset fence rather than OOM-killing the process; do not mistake a 512 MiB payload reservation for a 512 MiB RSS bound. Small injectable retention tests do not establish full-1-GiB performance or exact RSS, and VM/static UI tests are not browser proof.
...
```js
// Wrong: a default UTF-8 decoder strips a leading BOM from unredacted text.
const rawText = new TextDecoder('utf-8', { fatal: true }).decode(rawBytes);
// Correct: preserve exact valid UTF-8 on capture AND authenticated retrieval.
const rawText = new TextDecoder('utf-8', { fatal: true, ignoreBOM: true }).decode(rawBytes);
```
