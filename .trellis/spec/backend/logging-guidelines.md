...
- Raw mode does not instantiate `DetailRedactor` or snapshot configured credentials. Each client ingress/upstream attempt input/output/final downstream body captures exact valid UTF-8 up to 35 MiB, truncating only the diagnostic copy; invalid UTF-8 is explicitly omitted. Error-only raw captures request and response bodies of failed attempts only and releases successful attempt inputs. It never promotes body data to ordinary JSONL, metadata, service errors or model responses. A raw body itself may contain ANY credential; the admin sees it by explicit exception. No Header values or free-form account/model/provider/URL fields enter raw manifests/list summaries. Preserve a bounded fixed-field metadata projection, only safe enums/UUIDs/statuses and descriptors.
