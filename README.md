...
独立额度耗尽保护（与 `errorRules`、低额度缓存池无关）：真实上游 HTTP 429 且脱敏限长后的正文明确表示账号/订阅额度不足时，通过现有按账号去重、退避的额度作业强制复查；核验期间短暂暂停，失败或未知不会形成永久月封禁。只有**触发后新成功的月窗**参考剩余额严格低于 `quotaProtection.monthlyThresholdUsd`（全局默认每账号 $0.20，按参考月上限 $50 折算，严格小于；管理页支持 $0.01–$50.00 两位小数）才持久封禁；等于阈值不会封禁。月封禁跨重启、额度刷新及 Key/代理修改保留，必须在账号列表点击“解除月额度封禁”并确认可能再次失败；认证管理 API `POST /api/accounts/quota-recover` 只接受 `{ "id": "稳定账号 ID" }`，与普通规则恢复分离。5 小时/周已确认 100% 的临时阻塞需新成功完整三窗均小于 100% 才自动恢复，重置时间仅用于同一额度作业复查，失败/缺窗不可放行。全池阻塞时立即返回安全错误和 `Retry-After`。若确认月封禁时 `metadata.json` 原子写入失败，当前进程仍阻止该账号出站；认证管理页显示“写盘待重试”，复用额度定时器按有界退避重试写盘。**磁盘持续不可写时若进程在成功写盘前重启，未持久化的封禁无法恢复**；管理员应先修复存储并确认状态不再显示“写盘待重试”，再重启服务。该提示不会进入普通日志。这些金额只作社区参考，不代表真实账单或预计单请求成本。

额度通过账号 Bearer 后台读取半公开的 `GET /users/me/plan/usage-limits`，15 分钟后过期；常规额度排序中，失败、缺窗或接口变化均归为未知并回退普通调度；已确认的短窗口阻塞和月封禁不因刷新失败或缺窗放行，聊天请求不会等待额度刷新。统计页另行做**社区参考估算，非真实账单/官方承诺**：参考上限来自社区实测截图（每账号 5 小时约 $10、每周约 $25、每月约 $50），当前整池同档 Cline Pass 订阅的适用性由操作员确认，而非额度 API。各窗参考剩余 = 对应上限 × (1 − percentUsed/100)；当月剩余合计只加月窗，当前可用合计先逐账号取三窗各自美元参考剩余的最小值再求和，绝不能取百分比最小值乘 $50。两种合计独立计数：启用账号若最近成功快照在 15 分钟内且月窗有效即可计入月汇总，三窗均有效才计入当前可用；失败、过期、缺失、未配置不视为 0，已知 0 保留。四个预测时点（当前、+2h、+8h、+24h）只在有效重置时间晚于统计生成时间且不晚于目标时恢复相应窗口上限，其他情况保守沿用；未来假设没有新增消耗，缺失重置时注明预测下限和覆盖。统计显示生成/获取时间、两套纳入/排除/未知账号数；单账号百分比、重置时间、路由状态仍分别展示。若将来混入不同订阅档位，必须先重新评审整池适用性与参考上限，不能将估值冒充官方余额。账号成功率按请求/账号去重，模型×Provider 按每个具名真实 attempt 记录；无样本为 null，冷却、硬隔离和禁用作为独立状态展示。
...
页面金额为 **参考用量等值（USD），非实际扣费**，与上文社区参考额度完全不同。冻结的 ClinePass 官方参考价来自 https://docs.cline.bot/getting-started/clinepass ，本项目版本 `clinepass-2026-09-24-v1`、采集日 `2026-09-24`，**官方生效时间未知**。仅 `cline-pass/kimi-k3`（input/output/cached-read：$3/$15/$0.30 每百万 Token）、`cline-pass/glm-5.3`（$1.40/$4.40/$0.26）、`cline-pass/deepseek-v4-flash`（谷/峰 $0.22/$0.66/$0.007 至 $0.44/$1.32/$0.014）及 `cline-pass/deepseek-v4-pro`（谷/峰 $0.66/$1.98/$0.022 至 $1.32/$3.96/$0.044）报价。DeepSeek 档位不可判定，只显示峰谷参考区间；上游未给完整、非负、一致的 input/output/cached-read usage，或其他模型、缓存写入价缺匹配的计数，均不能计算，绝不按 0 推算。估算使用 `(input−cachedRead)×inputPrice + cachedRead×readPrice + output×outputPrice`，除以一百万；费用及价格版本按请求写入现有 `metadata.json` 分钟桶，未来价格版本不会追溯改价；最多保留 8 个价格版本、各 50,000 个渠道用量/估值单元，裁剪与溢出均有覆盖提示。金额只表示有证据的已计请求，不代表完整账单、渠道实际成本或包月额外扣费。
