...
export class BodyCapture {
  ...
  add(chunk, encoding) {
    ...
    if (!this.budget.reserve(size * 2)) { this.limited = true; this.limitReason = 'captureBudget'; this.discard(); return; }
    let copied = size;
    if (size) {
  ...
  materialize(redactor) {
    ...
    if (this.bytes && !this.limited) {
      try {
        ...
        if (Buffer.byteLength(text) > this.bytes && !this.budget.reserve((Buffer.byteLength(text) - this.bytes) * 2)) { this.limitReason = 'captureBudget'; text = ''; state = 'resource-limited'; }
...
export class DetailRoot {
  ...
  constructor(req, res, store, secrets = [], { profile = 'full', requestId = randomUUID() } = {}) {
    ...
    this.profile = profile === 'error' ? 'error' : 'full'; this.nextAttemptIndex = 0;
    ...
    this.headers = req.headers; this.attempts = []; this.closed = false; this.model = ''; this.responseHeaders = {}; this.status = null; this.omittedAttempts = 0;
    ...
    this.input = new BodyCapture(); this.output = new BodyCapture();
  ...
  attempt({ token = null, headers = {}, body = '', account = null, proxyUrl = '', method = 'POST', url, model = '', provider = null }) {
    ...
    if (!attemptModel || provider === null) try {
    ...
    const common = { ...identity, ts: Date.now(), method, url: this.redactor.text(this.redactor.url(url)), accountId: account?.id || null, accountName: account?.name || null, model: attemptModel, provider: attemptProvider, headers, responseHeaders: {}, status: null, outcomeStatus: null, captureState: 'pending', state: 'interrupted', failed: false };
    const attempt = this.profile === 'full'
      ? { ...common, input: new BodyCapture(), output: new BodyCapture() }
      : { ...common, requestSource: body };
    Object.defineProperty(attempt, 'root', { value: this });
    if (attempt.input) { attempt.input.add(body); attempt.input.end(); }
    if (!this.model) this.model = attemptModel;
    this.attempts.push(attempt); return attempt;
  }
  settleAttempt(attempt, { failed = false, httpStatus = null, outcomeStatus = null, responseHeaders = {}, responseBody, responseComplete = true, captureState = failed ? 'response-error' : 'success' } = {}) {
    ...
    if (!failed) {
      ...
      if (captureState !== 'stream-started') delete attempt.requestSource;
      return true;
    }
    ...
    attempt.discovery = new BodyCapture();
    if (requestSource !== undefined && requestSource !== null) { attempt.discovery.add(requestSource); attempt.discovery.end(); }
    if (responseBody !== undefined && responseBody !== null) {
      attempt.output = new BodyCapture(); attempt.output.add(responseBody); if (responseComplete) attempt.output.end();
    }
    return true;
  }
  finalize() {
    if (this.closed) return; this.closed = true;
    if (this.profile === 'error') for (const attempt of this.attempts) delete attempt.requestSource;
    const selectedAttempts = this.profile === 'error' ? this.attempts.filter((attempt) => attempt.failed) : this.attempts;
    const captures = this.profile === 'error'
    ...
    if (this.profile === 'error' && !selectedAttempts.length) { release(); return; }
    void this.store.publish({ generation: this.generation, ts: this.ts, requestId: this.requestId, requireOpen: this.profile === 'full', release, produce: () => {
      this.redactor.learnHeaders(this.headers);
      if (this.profile === 'full') this.redactor.learnHeaders(this.responseHeaders);
