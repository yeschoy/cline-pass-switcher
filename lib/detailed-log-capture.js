...
export class DetailRedactor {
  ...
  add(value) {
    ...
    const forms = [value]; try { forms.push(decodeURIComponent(value)); } catch {}
  ...
  url(value, partial = false) {
    try {
      ...
      for (const [key, v] of [...u.searchParams]) if (credentialName(key)) { this.unsafe ||= partial; this.add(v); u.searchParams.set(key, REDACTED); }
      return u.toString();
    } catch {
      ...
      for (const [key, secret] of new URLSearchParams(String(value).split('?')[1]?.split('#')[0] || '')) if (credentialName(key)) this.add(secret);
      return '[OMITTED: malformed URL]';
    }
  }
  learnHeaders(headers, discoverWrapped = true) {
    for (const [key, value] of Object.entries(headers || {})) if (credentialName(key)) {
      for (const v of Array.isArray(value) ? value : [String(value)]) {
        ...
        if (/cookie/.test(normalizeCredentialName(key))) for (const part of (/(?:^|-)set-cookies?$/.test(normalizeCredentialName(key)) ? v.split(';').slice(0, 1) : v.split(';'))) if (part.includes('=')) {
  ...
  text(value, partial = false) {
    ...
    // Ambiguous escaped text can hide a credential whose decoded echo appears
    // elsewhere. Fence discovery for the whole group, not just this string.
    if (/\\u[0-9a-f]{4}|\\x[0-9a-f]{2}/i.test(text)) { this.unsafe = true; return '[OMITTED: ambiguous escaped text]'; }
    ...
    return this.unsafe ? '[OMITTED: incomplete credential discovery]' : this.known(parts.join(''));
  }
  known(text) {
    if (this.limited || text.length > MAX_BODY_BYTES) { this.limited = true; return '[OMITTED: redaction resource limit]'; }
  ...
  headers(headers) {
    ...
    return Object.fromEntries(Object.entries(headers || {}).map(([key, value]) => [this.text(key), credentialName(key) ? REDACTED : this.text(Array.isArray(value) ? value.join('\n') : value)]));
  }
  json(value) {
    if (this.unsafe) return '[OMITTED: incomplete credential discovery]';
    ...
    return Object.fromEntries(Object.entries(value).map(([key, v]) => [this.text(key), credentialName(key) ? REDACTED : this.json(v)]));
  }
  partialText(text) {
  ...
  prefix(text) {
    ...
    if (/^[\[{]/.test(text.trimStart())) {
      ...
      for (let i = 0; i < text.length; i++) {
        ...
        else if (char === ':') { if (!scope) throw new Error('invalid JSON prefix'); scope.fieldCredential = credentialName(JSON.parse(text.slice(quoteStart, quoteEnd + 1))); }
