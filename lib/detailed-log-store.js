...
export class DetailedLogStore {
  constructor({ dir, maxAgeMs = MAX_AGE_MS, maxTotalBytes = MAX_TOTAL_BYTES, maxInventoryEntries = MAX_INVENTORY_ENTRIES, maxInventoryBytes = MAX_INVENTORY_BYTES, now = Date.now, io = fs } = {}) {
    ...
    this.health = { failures: 0, dropped: 0, corrupt: 0, lastFailure: null, dropReasons: Object.fromEntries(DETAIL_DROP_REASONS.map((reason) => [reason, 0])) };
  ...
  publish({ generation, ts, requestId, produce, release, requireOpen = false }) {
    ...
    return this.serial(async () => {
      ...
      try {
        if (generation !== this.generation || ts <= this.now() - this.maxAgeMs || !validDetailId(requestId)) { this.recordDrop('storeStale'); return false; }
        await this.expire();
        ...
        if (generation !== this.generation || ts <= this.now() - this.maxAgeMs) { this.recordDrop('storeStale'); return false; }
        if (requireOpen) {
