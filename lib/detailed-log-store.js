...
export class DetailedLogStore {
  ...
  validate(group, id, raw = false, directoryTs = null) {
    ...
    if (raw) {
      // Never promote free-form Header, URL, account or model data from a raw manifest.
      const requestKeys = ['requestId','ts','method','pathname','profile','model','accounts','status','httpStatus','outcomeStatus','result','complete','state','captureState','attemptCount','omittedAttempts','requestBody','responseBody'];
      const attemptKeys = ['callId','attemptIndex','ts','method','status','httpStatus','outcomeStatus','captureState','state','requestBody','responseBody'];
      ...
      if (Object.keys(group.request).some((key) => !requestKeys.includes(key)) || group.request.model !== '' || group.request.accounts?.length !== 0 || !['POST','GET'].includes(group.request.method) || !paths.has(group.request.pathname) || !statusOK(group.request.status) || !statusOK(group.request.httpStatus) || !statusOK(group.request.outcomeStatus) || group.request.result !== null && group.request.result !== undefined && !['success','failed','client_cancelled'].includes(group.request.result) || !stateOK(group.request.state) || typeof group.request.complete !== 'boolean' || group.request.omittedAttempts !== undefined && (!Number.isSafeInteger(group.request.omittedAttempts) || group.request.omittedAttempts < 0) || group.request.captureState !== undefined && !stateOK(group.request.captureState) || !Number.isSafeInteger(group.request.attemptCount) || group.request.attemptCount !== group.attempts.length || group.attempts.some((attempt) => !attempt || Object.keys(attempt).some((key) => !attemptKeys.includes(key)) || !statusOK(attempt.status) || !statusOK(attempt.httpStatus) || !statusOK(attempt.outcomeStatus) || !stateOK(attempt.state) || !stateOK(attempt.captureState) || !['POST','GET'].includes(attempt.method) || !Number.isSafeInteger(attempt.ts) || attempt.ts < 0)) throw new SyntaxError('unsafe raw metadata');
