...
test('canonical scoped error rules match status/body/header, isolate state, persist hard quarantine and project direct rates', async (t) => {
  ...
  response=await call('only-cooling');assert.equal(response.status,429);response=await call('only-cooling');assert.equal(response.status,503);assert.ok(Number(response.headers['retry-after'])>=1);const coolingLog=await waitUntil(async()=>{const page=await(await fetch(`http://127.0.0.1:${port}/api/logs/requests?requestedModel=only-cooling&limit=1`)).json();return page.items?.[0]?.status===503&&page.items[0];},5000,'only-cooling 503 request log row published');assert.equal(coolingLog.errorCategory,'routing');assert.equal(coolingLog.upstreamStatus,null);assert.deepEqual(coolingLog.attempts,[]);
